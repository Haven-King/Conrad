...
package dev.inkwell.vivid.api.widgets;

import dev.inkwell.vivid.api.DrawableExtensions;
import dev.inkwell.vivid.api.screen.ConfigScreen;
import dev.inkwell.vivid.api.screen.TooltipAccess;
import dev.inkwell.vivid.impl.VividConfig;
...
public abstract class WidgetComponent implements Element, DrawableExtensions, TickableElement, TooltipAccess {
    ...
    @Override
    public void tick() {
        if (VividConfig.Animations.ENABLED.getValue()) {
            if (isMouseOver(this.lastMouseX, this.lastMouseY)) {
                this.hoverOpacity = Math.min(1F, VividConfig.Animations.SPEED.getValue() + this.hoverOpacity);
            } else {
                this.hoverOpacity = Math.max(0F, this.hoverOpacity - VividConfig.Animations.SPEED.getValue());
            }
        } else {
