    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (VividConfig.Animations.ENABLED.getValue()) {
            if (isMouseOver(mouseX, mouseY) && (this.parent.getFocused() == null || this.parent.getFocused() == this)) {
                hoverOpacity = Math.min(1F, VividConfig.Animations.SPEED.getValue() + hoverOpacity);
            } else {
                hoverOpacity = Math.max(0F, hoverOpacity - VividConfig.Animations.SPEED.getValue());
            }
        } else {
