    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (VivianConfig.Animations.ENABLED.getValue()) {
            if (isMouseOver(mouseX, mouseY) && (this.parent.getFocused() == null || this.parent.getFocused() == this)) {
                hoverOpacity = Math.min(1F, VivianConfig.Animations.SPEED.getValue() + hoverOpacity);
            } else {
                hoverOpacity = Math.max(0F, hoverOpacity - VivianConfig.Animations.SPEED.getValue());
            }
        } else {
