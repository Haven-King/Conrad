...
import dev.inkwell.vivian.api.screen.TooltipAccess;
import dev.inkwell.vivian.impl.VivianConfig;
...
public abstract class WidgetComponent implements Element, DrawableExtensions, TickableElement, TooltipAccess {
    ...
    @Override
    public void tick() {
        if (VivianConfig.Animations.ENABLED.getValue()) {
            if (isMouseOver(this.lastMouseX, this.lastMouseY)) {
                this.hoverOpacity = Math.min(1F, VivianConfig.Animations.SPEED.getValue() + this.hoverOpacity);
            } else {
                this.hoverOpacity = Math.max(0F, this.hoverOpacity - VivianConfig.Animations.SPEED.getValue());
            }
        } else {
