...
import dev.monarkhes.vivid.util.StronglyTypedCollection;
...
public class ParentConfigValue<T extends StronglyTypedCollection<?, V, ?>, V> extends ConfigValue<T> {
    ...
    @Override
    public ParentConfigValue<T, V> sync(SyncType syncType) {
        super.sync(syncType);
        return this;
    }
    ...
    public ConfigValue<V> getChildValue() {
        ...
        if (this.getSyncType() != SyncType.NONE) child.sync(this.getSyncType());
